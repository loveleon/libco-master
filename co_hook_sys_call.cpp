...
/* socket - 被hook后的socket函数, 主要是为套接字fd分配对应的rpchook_t类型的内存空间, 
并往g_rpchook_socket_fd中添加该内存空间的地址(指针指向的变量未全部初始化) */
int socket(int domain, int type, int protocol)
{
	...
	/* 为fd分配rpchook_t类型的内存空间, 其中存储套接字hook信息, 
	并将其加入套接字hook信息数组g_rpchook_socket_fd中*/
...
ssize_t read( int fd, void *buf, size_t nbyte )
{
	...
	// poll如果被hook,则调用co_poll向内核注册, 
	//co_poll中会切换协程, 协程被恢复时将会从co_poll中的挂起点继续运行
...
/* setsockopt - 被hook后的setsockopt函数, 主要是初始化(g_rpchook_socket_fd中)套接字fd对应
的rpchook_t类型变量的read_timeout和write_timeout成员 */
...
/* fcntl - 被hook后的fcntl函数, 主要是初始化(g_rpchook_socket_fd中)
套接字fd对应的rpchook_t类型变量的user_flag成员 */
