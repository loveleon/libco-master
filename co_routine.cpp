...
struct stCoRoutineEnv_t
{
	...
	stCoRoutine_t *pCallStack[ 128 ];//记录当前创建的协程//记录具体某个协程
	// 该线程内嵌套创建的协程数量, 即pCallStack数组中元素的数量
	int iCallStackSize;//记录当前一共创建了多少个协程
	// 该线程内的epoll实例(套接字通过该结构内的epoll句柄向内核注册事件), 也用于该线程的事件循环eventloop中
...
void co_log_err( const char *fmt,... )/* co_log_err - 协程日志输出 */
{
}
		...
		pfn_co_routine_t pfn,void *arg )
{
	...
	//1111 1111 1111 如果低12位非零，则将低十二位置零，并进一位，
	//即stack_size应该是4kb的整数倍 x86一个内存页大小是4kb,这里考虑到内存对齐的问题
	...
	lp->env = env;//记录下这个co_routine的上文// 初始化协程中的线程环境
	lp->pfn = pfn;//记录下执行的函数// 初始化协程函数
	lp->arg = arg;//记录下参数// 初始化协程函数的参数
...
{
	if( !co_get_curr_thread_env() ) //1如果得不到当前线程的上下文
	{
		co_init_curr_thread_env();//则初始化上下文// 初始化协程环境(协程环境其实就是调度器)
	}
...
void co_resume( stCoRoutine_t *co )
{
	...
	env->pCallStack[ env->iCallStackSize++ ] = co;//设置co为运行的线程// 将协程co加入到协程环境的协程数组末尾
	co_swap( lpCurrRoutine, co );// 保存当前上下文到lpCurrRoutine->ctx, 并切换到新的上下文co->ctx
...
void co_yield_env( stCoRoutineEnv_t *env )
{
	...
	co_swap( curr, last);// 切换到上次被切换出去的协程last
}
